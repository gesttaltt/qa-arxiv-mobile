...
- Loading time: 2s (Android) -- consistent with baseline
...
|-----------|---------|-----|-------|
| Descriptive error message shown | [x] Pass [ ] Fail | N/A | Message: "No internet connection" |
| No crash or freeze | [x] Pass [ ] Fail | N/A | App remained responsive |
| No infinite loading spinner | [x] Pass [ ] Fail | N/A | Timeout ~5s then error shown |
| App interactive during offline | [x] Pass [ ] Fail | N/A | All navigation still works |
| Successful search after restore | [x] Pass [ ] Fail | N/A | Results match baseline |
| Cached results visible (if applicable) | [x] Pass [ ] Fail | N/A | Previous results still accessible |
...
- **iOS:** [ ] Not executed - `TC004_SearchOfflineBehavior_iOS_Pass.gif` is a placeholder (Android recording, "Pending macOS environment" banner)

### Screenshots:
- **Offline Error:** [ ] Mislabeled -- `evidence/screenshots/TC004_offline_error.png` actually shows a normal paper detail screen, not the offline error state. No accurate offline-error screenshot was captured; the GIF is the only evidence of the actual error message.

### Evidence Location:
- **Android:** `evidence/android/TC004_SearchOfflineBehavior_Android_Pass.gif`
- **iOS (placeholder only):** `evidence/ios/TC004_SearchOfflineBehavior_iOS_Pass.gif`
...
**Summary Notes:**
Offline error handling works correctly on Android. The app detects network loss, shows a clear error message, does not crash, and recovers fully when connectivity is restored. Some improvement opportunity around cached result display noted. iOS was not executed — see Evidence Collected for the placeholder status of the "iOS" GIF.
