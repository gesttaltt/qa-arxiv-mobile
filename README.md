...
- Built API test coverage at two layers: a Postman collection (8 requests, `pm.test()` assertions) covering TC001, TC002, Equivalence Partitioning (author field, pagination offset, cross-request `au:` vs `all:` comparison using `pm.sendRequest` + `pm.collectionVariables`), Boundary Value Analysis (max\_results, pagination edge), and Error Guessing (XSS injection); and 74 pytest tests for CI — API integration, mock-based SLA validation, API contract tests for Favorites and PDF links (TC005–TC007), retry-logic unit tests, and mock-based POM unit tests covering all page object methods (100% statement coverage)
- Authored a GitHub Actions CI pipeline running on every push — Python linting (Black, Ruff, mypy), pytest with `--cov-fail-under=100` quality gate, Codecov coverage reporting (100% coverage via mock-based POM unit tests), and Markdown/YAML validation; mirrored as Azure Pipelines config for ADO environments
...
> Built an end-to-end QA portfolio on a real React Native app — 11 manual test cases, 74 automated tests (API, BDD/Gherkin, Appium POM + mock unit tests, 100% coverage), Postman collection, 7 defect reports, ADO traceability, and a GitHub Actions CI pipeline with Codecov coverage gate.
