...
class TestDataFormats:
    ...
    def test_published_date_is_iso8601(self) -> None:
        ...
        for entry in entries:
            ...
            assert self.ISO8601_PATTERN.match(
                published.strip()
    ...
    def test_entry_id_is_valid_arxiv_url(self) -> None:
        ...
        for entry in entries:
            ...
            assert self.ARXIV_ID_PATTERN.match(
                entry_id.strip()
    ...
    def test_no_duplicate_entry_ids(self) -> None:
        ...
        ids = [(entry.findtext(f"{{{ATOM_NS}}}id") or "").strip() for entry in entries]
        assert len(ids) == len(
            set(ids)
